...
begin
 setlength(aktual,0);
 traverseKDTreelinDNT(r1,r2,aktual,KDData,KDTree,tris);
...
begin
	...
	begin
		...
		if( (bv[i].lu - 0.5) < minu ) then
                     minu := round(bv[i].lu - 0.5);
                if( (bv[i].lv - 0.5) < minv ) then
                     minv := round(bv[i].lv - 0.5);
                if( (bv[i].lu - 0.5) > maxu ) then
                     maxu := round(bv[i].lu + 0.5);
                if( (bv[i].lv - 0.5) > maxv ) then
                     maxv := round(bv[i].lv + 0.5);
	end;
	...
	for  y := minv to maxv do
	begin
		for x:=minu to maxu do
		begin
			...
			begin
                        ...
                        if vil3*lghtszor>0.4 then vil3:=0.4/lghtszor;
...
begin
 ...
 begin
  ...
  begin
   getmem(buf,1024);
   strcopy(buf,mats[i].pTextureFilename);
   str:=string(mats[i].pTextureFilename);
   str:=ExtractFileName(str);
   buf:=PChar(str);
   strcopy(mats[i].pTextureFilename,buf);
   if (mats[i].pTextureFilename='lght.bmp') or
      (mats[i].pTextureFilename='lght.jpg') then lght:=i;
  end;
  ...
  szetvalaszt:=(sqrt(length(charts))/siz)*20;
  ...
  begin
   with pvert[pind[i*3]] do
    canvas.moveto(round(lu*256),round(lv*256));
   with pvert[pind[i*3+1]] do
    canvas.lineto(round(lu*256),round(lv*256));
   with pvert[pind[i*3+2]] do
    canvas.lineto(round(lu*256),round(lv*256));
   with pvert[pind[i*3]] do
    canvas.lineto(round(lu*256),round(lv*256));
   with pvert[pind[i*3]] do
   canvas.pixels[round(lu*256),round(lv*256)]:=clblack;
  end; //}
   ...
   for x:=0 to siz-1 do
    ...
    if (((x+y) and 1)=0) xor ((i and 1)=0) then
     ...
     begin
      ...
      if x<255 then if bmp.canvas.pixels[x+1,y]<>clfuchsia then
       begin
        inc(hol);most:=most+bmp.canvas.pixels[x+1,y] and $FF;
       end;
      ...
      if y<255 then if bmp.canvas.pixels[x,y+1]<>clfuchsia then
       begin
        inc(hol);most:=most+bmp.canvas.pixels[x,y+1] and $FF;
       end;
      inc(hol2,hol);
